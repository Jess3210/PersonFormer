...
class ToTensor():
    ...
    def __call__(self, sample):
        ...
        img = np.transpose(image, (0, 1, 4, 2, 3)) #from B, T, W, H, C to B, T, C, H, W - notice: Batch = 1 as it is per list
        return {'image': torch.squeeze(torch.from_numpy(np.array(img))), 'groundtruth' : torch.from_numpy(np.array(sample['groundtruth'])), 'name' : sample['name']}
...
class ChaLearnDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        sample = {'image': extractedFrames, 'groundtruth' : personalityTraits, 'name' : vid}
