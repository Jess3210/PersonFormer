...
class visionFusion(torch.nn.Module):
    def __init__(
        self,
        num_classes = 5,
        faceCheckpoint = './checkpoint/visionFullTrainingLR0.01/parameters_finetuning.pth', #if weights are available
        personCheckpoint = './checkpoint/visionFullTrainingPersonLR0.001/parameters_finetuning.pth', #if weights are available
        input_vision=768,
        output_vision=512,
        d_model=512,
        nhead = 8,
    ...
    def forward(self, imageFace, imagePerson): 
        ...
        fused = self.dropout(
            ...
            self.fusionFacePerson(combined)
            )
        )
        residual = fused
        ...
        logits = self.fc(facePerson_features)
