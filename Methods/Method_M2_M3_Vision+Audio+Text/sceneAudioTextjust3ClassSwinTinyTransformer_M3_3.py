...
class visionAudioTextFusion(torch.nn.Module):
    def __init__(
        self,
        num_classes = 5,
        faceCheckpoint = './checkpoint/visionFullTrainingLR0.01/parameters_finetuning.pth',
        personCheckpoint = './checkpoint/visionFullTrainingPersonLR0.001/parameters_finetuning.pth',
        sceneCheckpoint = './checkpoint/visionFullTrainingScene/parameters_finetuning.pth',
        textCheckpoint = './checkpoint/text_parameters_finetuning.pth',
        audioCheckpoint = './checkpoint/audio_1s_parameters_finetuning.pth',
        input_vision=768,
        output_vision=512,
        d_model=512,
        nhead = 8,
    ...
    def forward(self, imageFace, imagePerson, imageScene, audio, text): 
        ...
        facePerson_features += residual
        facePerson_features = self.layer_norm(facePerson_features)
        logits = self.output(facePerson_features)
...
class visionAudioTextFusionClass():
    def __init__(self, optimizer='sgd', lossfnct='mse', model='inst', learningrate=1e-5, device='cuda'):
        ...
        # checkpointLoader = torch.load('sceneAudioText/checkpoint/sceneAudioText_parameters_checkpoint.pt')
        ...
        # self.loss_values_train = openFilePkl('sceneAudioText/lossValuesTrain.pickle')
        # self.loss_values_val = openFilePkl('sceneAudioText/lossValuesVal.pickle')
        # #self.loss_values_test = openFilePkl('visionAudioTextM1/lossValuesTest.pickle')
        # self.mse_class_train = openFilePkl('sceneAudioText/mseClassTrain.pickle')
        # self.mse_class_val = openFilePkl('sceneAudioText/mseClassVal.pickle')
        # #self.mse_class_test = openFilePkl('visionAudioTextM1/mseClassTest.pickle')
        # self.traitAccTrain = openFilePkl('sceneAudioText/traitAccTrain.pickle')
        # self.meanAvgAccAllTrain = openFilePkl('sceneAudioText/meanAvgAccAllTrain.pickle')
        # self.traitAccVal = openFilePkl('sceneAudioText/traitAccVal.pickle')
        # self.meanAvgAccAllVal = openFilePkl('sceneAudioText/meanAvgAccAllVal.pickle')
    ...
    def train(self, train_dl, epoch):
        ...
        torch.save({
            ...
            'loss': loss
            }, self.pathRes+'/checkpoint/sceneAudioText_parameters_checkpoint.pt')
