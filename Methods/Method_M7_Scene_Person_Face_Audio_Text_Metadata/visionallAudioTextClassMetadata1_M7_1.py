...
class visionAudioTextFusion(torch.nn.Module):
    def __init__(
        self,
        num_classes = 5,
        faceCheckpoint = './r2plus1_scene_parameters_checkpoint.pth', #load checkpoints - needs to be added in folder
        personCheckpoint = './r2plus1_scene_parameters_checkpoint.pth', #load checkpoints - needs to be added in folder
        sceneCheckpoint = './r2plus1_scene_parameters_checkpoint.pth', #load checkpoints - needs to be added in folder

        textCheckpoint = './text_parameters_finetuning.pth', #load checkpoints - needs to be added in folder
        #audioCheckpoint = './audio_1s_parameters_finetuning.pth',
        ...
        self.fusion_avtfp_hidden = torch.nn.Linear(
            in_features=1029, #1024 + 5 metadata
            out_features=512
        )
        #scene + text + audio
        #layer1
    ...
    def forward(self, imageFace, imagePerson, imageScene, audio, text, metadata): 
        ...
        combinedVAT = torch.cat(
            [visionScene, audio_features, textfeat], dim=1
        )
        ...
        combinedVATFP = torch.cat(
            [fusedAVT_features, facePerson_features, metadata], dim=1
        )
