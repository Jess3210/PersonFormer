...
class visionAudioTextFusion(torch.nn.Module):
    def __init__(
        ...
        self.pos_encoder_personFace = PositionalEncoding(d_model=1, dropout=dropoutTransformer, max_len=output_vision)
        ...
        self.pos_encoder_avt = PositionalEncoding(d_model=1, dropout=dropoutTransformer, max_len=512)
        ...
        self.pos_encoder_avtfp = PositionalEncoding(d_model=1, dropout=dropoutTransformer, max_len=512)
        #scene + text + audio
        #layer1
        self.layer_norm = nn.LayerNorm(d_model)
    ...
    def forward(self, imageFace, imagePerson, imageScene, audio, text, metadata): 
        ...
        fusedAVT = self.pos_encoder_avt(fusedAVT)
        ...
        hiddenLayerFacePerson = self.pos_encoder_personFace(hiddenLayerFacePerson)
        ...
        hiddenLayerVATFP = self.pos_encoder_avtfp(hiddenLayerVATFP)
