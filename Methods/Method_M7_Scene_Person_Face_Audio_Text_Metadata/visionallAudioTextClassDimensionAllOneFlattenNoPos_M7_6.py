...
class visionAudioTextFusion(torch.nn.Module):
    def __init__(
        ...
        #self.vision_module_scene.load_state_dict(torch.load(sceneCheckpoint)) load pretrained weights 
        
        #load face model checkpoint
        self.vision_module_face = visionClass()
        #self.vision_module_face.load_state_dict(torch.load(faceCheckpoint)) load pretrained weights 

        #load person model checkpoint
        self.vision_module_person = visionClass()
        #self.vision_module_person.load_state_dict(torch.load(personCheckpoint)) load pretrained weights 
        ...
        self.fctransformer1_1 = nn.Linear(d_model2, dimFeedforward)
        self.fctransformer1_2 = nn.Linear(dimFeedforward, d_model2)

        #layer2
        self.layer_norm1 = nn.LayerNorm(d_model2)
        self.self_attn2 = nn.MultiheadAttention(d_model2, nhead, dropout=dropoutTransformer, batch_first=True)
        self.fctransformer2_1 = nn.Linear(d_model2, dimFeedforward)
        self.fctransformer2_2 = nn.Linear(dimFeedforward, d_model2)

        self.flatten = nn.Flatten()
        self.audioHidden = nn.Linear(15*128, d_model)
        ...
        self.outputHidden = nn.Linear(5*(d_model+8), d_model)
    ...
    def forward(self, imageFace, imagePerson, imageScene, audio, text, metadata): 
        ...
        audio_hidden = self.dropout(
            torch.nn.functional.relu(self.audioHidden(audio_features)))
        ...
        audio_features = torch.cat([audio_hidden, metadata], dim=1)
        textfeat = torch.cat([textfeat, metadata], dim=1)
        combinedVATFP = torch.stack(
            [visionScene, audio_features, textfeat, visionFace, visionPerson], dim=0
        )
        combinedVATFP = torch.permute(combinedVATFP, (1, 0, 2))

        residual = combinedVATFP
        features_all = self.self_attn7(combinedVATFP, combinedVATFP, combinedVATFP)[0]
        ...
        features_all = self.flatten(features_all)
        
        featuresAll_hidden = self.dropout(
            torch.nn.functional.relu(self.outputHidden(features_all)))

        logits = self.output(featuresAll_hidden)
