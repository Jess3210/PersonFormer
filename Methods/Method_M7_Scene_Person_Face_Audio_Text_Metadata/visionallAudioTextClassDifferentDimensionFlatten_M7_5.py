...
class visionAudioTextFusion(torch.nn.Module):
    def __init__(
        ...
        self.audioHidden = nn.Linear(15*512, d_model)
        ...
        self.personFaceHidden = nn.Linear(2*(d_model+8), d_model)
        ...
        self.avtHidden = nn.Linear(3*(d_model+8), d_model)

        self.pos_encoder_avtfp = PositionalEncoding(d_model=d_model, dropout=dropoutTransformer, max_len=2)
        #scene + text + audio
        #layer1
        ...
        self.outputHidden = nn.Linear(2*d_model, d_model)
    ...
    def forward(self, imageFace, imagePerson, imageScene, audio, text, metadata): 
        ...
        fusedAVT_features = self.flatten(fusedAVT_features)
        
        avt_hidden = self.dropout(
            torch.nn.functional.relu(self.avtHidden(fusedAVT_features)))
        ...
        facePerson_features = self.flatten(facePerson_features)
        
        facePerson_hidden = self.dropout(
            torch.nn.functional.relu(self.personFaceHidden(facePerson_features)))
        #projection layer scene + audio + text + metadata

        combinedVATFP = torch.stack(
            [avt_hidden, facePerson_hidden], dim=0
        )
