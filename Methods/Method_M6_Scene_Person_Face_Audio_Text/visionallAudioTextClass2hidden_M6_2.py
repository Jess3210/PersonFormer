...
class visionAudioTextFusion(torch.nn.Module):
    def __init__(
        ...
        self.fusion_avt_hidden = torch.nn.Linear(
            in_features=1024, 
            out_features=1024
        )
        ...
        self.fusion_avtfp_hidden_2 = torch.nn.Linear(
            in_features=1024, 
            out_features=1024
        )
    ...
    def forward(self, imageFace, imagePerson, imageScene, audio, text): 
        ...
        #hidden layer scene + audio + text
        hiddenLayerAVT = self.dropout(
            torch.nn.functional.relu(
                self.fusion_avt_hidden(fusedAVT)
            )
        )
        ...
        combinedVATFP = torch.cat(
            [hiddenLayerAVT, hiddenLayerFacePerson], dim=1
        )
        ...
        hiddenLayerVATFP_2 = self.dropout(
            torch.nn.functional.relu(
                self.fusion_avtfp_hidden_2(hiddenLayerVATFP)
            )
        )
        logits = self.output(hiddenLayerVATFP_2)
