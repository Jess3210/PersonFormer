...
from architectures.visionFusionClass_2_transformer_layer import visionFusion #trained classes are necessary
from architectures.sceneAudioTextjust3Class import visionAudioTextFusion as sceneAudioTextFusion #trained classes are necessary
...
class visionAudioTextFusion(torch.nn.Module):
    def __init__(
        self,
        num_classes = 5,
        sceneAudioTextCheckpoint = 'checkpoint/sceneAudioText/parameters_finetuning.pth',
        personFaceCheckpoint = 'checkpoint/visionFacePersonFusionTransformer_2/parameters_finetuning.pth',
        output_vision=512,
        d_model=512,
        nhead = 8,
        dimFeedforward = 2048,
        ...
        self.sceneAudioText = sceneAudioTextFusion()
        self.sceneAudioText.load_state_dict(torch.load(sceneAudioTextCheckpoint))

        self.personFace = visionFusion()
        self.personFace.load_state_dict(torch.load(personFaceCheckpoint))
        
        #Freeze early layers
        for param in self.sceneAudioText.parameters():
            param.requires_grad = False

        for param in self.personFace.parameters():
            param.requires_grad = False

        self.sceneAudioText.output = nn.Linear(output_vision, output_vision, bias=True)
        self.personFace.fc = nn.Linear(output_vision, output_vision, bias=True)
    ...
    def forward(self, imageFace, imagePerson, imageScene, audio, text): 
        sceneAudioText = self.sceneAudioText(imageFace, imagePerson, imageScene, audio, text)
        personFace = self.personFace(imageFace, imagePerson)

        #projection layer scene + audio + text
        combinedVATFP = torch.cat(
            [sceneAudioText, personFace], dim=1
        )
        ...
        #facePerson_features = self.layer_norm(fused)
        features_all = self.self_attn1(hiddenLayerVATFP, hiddenLayerVATFP, hiddenLayerVATFP)[0]
        features_all += residual
        features_all = self.layer_norm1(features_all)
        residual = features_all
        features_all = self.dropoutTransf(torch.nn.functional.relu(
            self.fctransformer1_1(features_all)))
        features_all = self.dropoutTransf(
            torch.nn.functional.relu(self.fctransformer1_2(features_all)))
        features_all += residual
        features_all = self.layer_norm1(features_all)
        ...
        residual = features_all
        #facePerson_features = self.layer_norm(fused)
        features_all = self.self_attn2(features_all, features_all, features_all)[0]
        features_all += residual
        features_all = self.layer_norm1(features_all)
        residual = features_all
        features_all = self.dropoutTransf(torch.nn.functional.relu(
            self.fctransformer2_1(features_all)))
        features_all = self.dropoutTransf(
            torch.nn.functional.relu(self.fctransformer2_2(features_all)))
        features_all += residual
        features_all = self.layer_norm1(features_all)
