...
class visionAudioTextFusion(torch.nn.Module):
    def __init__(
        ...
        #not freeze child 4, 5,6
        # face
        child_counter = 0
        for child in self.vision_module_face.vision_module.children():
            if child_counter < 4:
        ...
        #person
        child_counter = 0
        for child in self.vision_module_person.vision_module.children():
            if child_counter < 4:
                for param in child.parameters():
                    param.requires_grad = False
                child_counter += 1
        ...
        #scene
        child_counter = 0
        for child in self.vision_module_scene.vision_module.children():
            if child_counter < 4:
                for param in child.parameters():
                    param.requires_grad = False
                child_counter += 1
